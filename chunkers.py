...
class Chunkers:
    ...
    def get_opennlp_chunker(self, language='en'):
        if not (self.opennlp_chunkers.has_key(language) and self.opennlp_chunkers[language]):
    ...
    def get_treetagger_chunker(self, language='en'):
        if not (self.treetagger_chunkers.has_key(language) and self.treetagger_chunkers[language]):
            dirname, filename = os.path.split(os.path.abspath(__file__))
    ...
    def __get_ner_chunker(self, chunkers_list, name_str, language):
        if not (chunkers_list.has_key(language) and chunkers_list[language]):
            dirname, filename = os.path.split(os.path.abspath(__file__))
