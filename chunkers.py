...
class Chunkers:
    ...
    def get_opennlp_chunker(self, language='en'):
        if not self.__has_key(self.__opennlp_chunkers, language):
            ...
            if not (os.path.exists(path_to_model) and os.path.isfile(path_to_model)):
                return None
            self.__opennlp_chunkers[language] = OpenNLPChunker(language=language,
    ...
    def get_ner_date_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_date_chunkers, 'date', language)

    def get_ner_location_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_location_chunkers, 'location', language)

    def get_ner_money_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_money_chunkers, 'money', language)

    def get_ner_organization_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_organization_chunkers, 'organization', language)

    def get_ner_percentage_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_percentage_chunkers, 'percentage', language)

    def get_ner_person_chunker(self, language='en'):
        return self.__get_ner_chunker(self.__ner_person_chunkers, 'person', language)

    def get_ner_time_chunker(self, language):
        return self.__get_ner_chunker(self.__ner_time_chunkers, 'time', language)

    def get_english_multichunker(self):
        if self.__ner_multi_chunker is None:
            language = 'en'
            dirname, filename = os.path.split(os.path.abspath(__file__))
            self.__ner_multi_chunker = OpenNERChunkerMulti(language=language,
                                                           path_to_bin=os.path.join(dirname, 'apache-opennlp', 'bin'),
